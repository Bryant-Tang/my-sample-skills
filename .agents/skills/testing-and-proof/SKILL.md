...
- Reuse the exact build method already defined by the `build-project` skill whenever IIS-backed verification may need fresh binaries.
- Reuse the exact IIS startup and shutdown method already defined by the `run-project` skill.
- Do not substitute different build commands, different ports, or foreground IIS Express startup.
- Resolve the browser target URL with [get-target-url.ps1](./scripts/get-target-url.ps1), which reuses the `run-project` IISUrl parsing logic.
- If `.agents/skill-scripts.psd1` key `TEST_LOCAL_STASH_SHA` is configured, apply that local-only git stash before each IIS-backed verification task.
...
6. For each IIS-backed browser task whose prerequisites are already satisfied, run this sequence as separate steps:
   ...
   - If `TEST_LOCAL_STASH_SHA` is configured, apply the named local-test stash with [apply-local-test-stash.ps1](./scripts/apply-local-test-stash.ps1).
   ...
   - Resolve the exact browser target URL with [get-target-url.ps1](./scripts/get-target-url.ps1).
   - Invoke `runSubagent` for that one verification task only. The subagent must execute the verification, save screenshot files under `screenshots/`, and write or overwrite the corresponding `test-n.md`.
   - Stop IIS Express using the `run-project` workflow resources.
   - If `TEST_LOCAL_STASH_SHA` is configured, revert the local-test stash changes with [revert-local-test-stash.ps1](./scripts/revert-local-test-stash.ps1).
7. For each non-browser task whose prerequisites are already satisfied, invoke `runSubagent` for that one verification task only. The subagent must write or overwrite the corresponding `test-n.md` with file plus line evidence.
...
- Browser-backed verification subagents may use browser tools and `run_playwright_code` to save screenshot files into the spec folder.
- Browser-backed verification subagents should use [get-target-url.ps1](./scripts/get-target-url.ps1) to resolve the base URL instead of reconstructing scheme or port themselves.
- Browser-backed verification subagents must embed screenshot paths in `test-n.md` with Markdown image syntax and use only unmodified system screens, following [evidence-checklist.md](./references/evidence-checklist.md).
