...
try {
    ...
    if ($settings.IisScheme -eq 'https' -and [string]::IsNullOrWhiteSpace($settings.ApplicationhostConfigFile)) {
        throw "Configured IISUrl uses HTTPS: $($settings.IisUrl)`nMissing RUN_IIS_APPLICATIONHOST_CONFIG_PATH in .agents/skill-scripts.psd1.`nProvide an applicationhost.config path so IIS Express can start with /config and /site."
    }
